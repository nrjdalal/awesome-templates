"""Shared governor policy (Tier B). Consumed by Claude/Codex/Antigravity hook adapters.
...
and ``.antigravity/hooks/_shared.py`` Gemini / Antigravity payload helpers)
remain per-tool and are not duplicated here.

Phase 5 (#124) consolidates duplicate helpers from Phase 2/3/4 hook scripts
into this single package. Hook scripts under ``.claude/hooks/``,
``.codex/hooks/``, and ``.antigravity/hooks/`` import from here as thin shims.
