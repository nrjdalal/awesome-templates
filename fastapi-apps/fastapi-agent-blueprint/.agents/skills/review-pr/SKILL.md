...
3. Resolve the review target, collect the diff + intent, and load the rule sources; on a
   re-review round, also extract prior rounds' `Summary Finding Ledger` and diff every key
   against the new head (Phase 0).
