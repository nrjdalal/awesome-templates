...
- Error Notification: Optional (NOTIFICATION_PROVIDER env var, Slack/Discord webhook adapters via providers.Selector + NoopNotificationClient fallback; fired from the global exception handlers only — server-only, #17; worker/admin coverage undecided in #310). Runbook: [`docs/operations/error-notifications.md`](../../docs/operations/error-notifications.md)
