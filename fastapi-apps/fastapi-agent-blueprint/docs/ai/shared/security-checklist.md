...
### Raw Provider Exceptions in Response Bodies and Alerts
- [ ] [When applicable][HIGH] No `except <ProviderError> as e:` block interpolates `{e}` into a `BaseCustomException` message
  - Detection condition: any adapter under `src/_core/infrastructure/` that catches a cloud-SDK or LLM-SDK exception — `ClientError` (botocore), provider errors from `pydantic_ai`, driver errors wrapped by `Database.session()`
  - Grep: `message=f"[^"]*\{e[c}]` and `f".*: \{e\}"` under `src/_core/infrastructure/` -> expect zero hits
  - Grep: `except ClientError as e:` under `src/_core/infrastructure/` -> every block must raise through a curating helper (`_storage_failure` in `object_storage.py`, the `Error.Code` extraction in `dynamodb_client.py` and `vectors/s3/client.py`), never build the message inline
  - The house pattern is a **split**, and both halves are required: the provider message goes to structlog, and the client gets `"<operation> failed [<ERROR_CODE>]"` with a real `error_code` and `from e`. A curated message with no `error_code` still degrades every failure to generic `CUSTOM_ERROR`
  - The provider message must not be a structlog **kwarg** either. `object_storage.py` records the error code as a field and leaves the message to the `from e` chain for `exc_info` rendering, because an S3 `Error.Message` names the object key — which for a user-uploaded file is often personal data
  - Not a finding: a curated *pre-branch* that maps a specific code to a 4xx (`NoSuchKey` -> 404 in `download_file`, `"404"` -> `False` in `file_exists`). Those are correct as long as the fall-through still curates
  - Reason: `ErrorResponse.message` is serialized unconditionally by `custom_exception_handler` — unlike `details`, it has **no `is_dev` gate** — and `ErrorNotifier` posts the same string to Slack/Discord, so one interpolation leaks to two destinations at once. A boto3 `AccessDenied` message names the calling IAM principal ARN (embedding the 12-digit AWS account id), the bucket and the key: the first two facts needed for a cross-account or role-confusion probe. This was #323 F5 across all six `except ClientError` blocks in `object_storage.py`; it was rated *medium* only because nothing reaches that module at HEAD, and it becomes a live disclosure the moment an adopter wires the first upload route — which is exactly what a blueprint invites
