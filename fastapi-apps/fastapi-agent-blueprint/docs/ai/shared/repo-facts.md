...
- `tools/check_harness_hook_surface.py`: local/pre-commit guard that rejects bare `python3` only in live agent hook execution surfaces (`.codex/hooks.json` command strings and executable lines in `.claude/hooks/*.sh`).
