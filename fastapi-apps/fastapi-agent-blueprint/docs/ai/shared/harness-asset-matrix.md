...
- **Notes**: Boundary contract (R0-C.3): this package owns *policy*; tool-specific runtime utilities (`.codex/hooks/_shared.py` git/subprocess helpers, per-tool `session_id()` resolution) remain per-tool. **Narrowed by #334**: the verify-log *helpers* — `VERIFY_PATTERNS`, `verify_log_path`, `append_verify_log`, `cleanup_stale_verify_logs` — moved into `governor/verify.py` once a second harness needed them. They take `session_id` as a parameter and own no session state, so the session-aware half stays per-tool; `.codex` and `.antigravity` keep their inline copies (ADR 045 parity, and #334 forbids harmonising further). `__all__` declared in `__init__.py` enforces stability — `test_governor_boundary.py::test_governor_all_does_not_drop_known_names` fails the build if a public name is removed without deliberate review.
...
| `.claude/hooks/verify-log.sh` | Overlay | Low | Low |
...
| `.claude/hooks/verify_log.py` | Overlay | Low | Low |
...
- **Current role**: Phase 5 (#124) thin shim. Pillar 7 logic imported from `.agents/shared/governor/completion_gate.py`; IC-11 Option A lifecycle imported from `.agents/shared/governor/markers.py`. `cleanup_stale_verify_logs` was a Codex-only runtime adapter because it depended on `verify_first.session_id()`. #334 lifted the helper into `governor/verify.py` with `session_id` as a parameter, so the dependency inverted: the shared helper is pure and each adapter supplies its own id. The Codex inline copy is unchanged.
...
If a future `Replace` candidate emerges, the threshold is: Keep+Overlay would otherwise force the asset into structural inconsistency with the Default Flow. None of the current 89 active assets meet that (the 90th matrix row is the retired `tools/check_g_closure.py` Drop, which is not a `Replace` candidate).
...
- 2026-08-05 — #334: added `.claude/hooks/verify-log.{sh,py}` Tier 3 rows — the first `PostToolUse` **Bash** matcher in `.claude`, without which the tree read verify-log state it never produced. Narrowed the R0-C.3 boundary: the verify-log helpers (`VERIFY_PATTERNS`/`verify_log_path`/`append_verify_log`/`cleanup_stale_verify_logs`) are now shared policy in `governor/verify.py` because they take `session_id` as a parameter; resolving that id stays per-tool, and the `.codex`/`.antigravity` inline copies are untouched. Total 88 → 90 (Tier 3 31 → 33).
