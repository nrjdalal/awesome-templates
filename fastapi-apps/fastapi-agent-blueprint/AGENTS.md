...
| Error Notification channel routing (#286) | `NOTIFICATION_WARNING_THRESHOLD` set, below `NOTIFICATION_SEVERITY_THRESHOLD`. It is the only switch — the per-tier `NOTIFICATION_CRITICAL_WEBHOOK_URL` / `NOTIFICATION_WARNING_WEBHOOK_URL` overrides do not enable routing on their own and are rejected at boot without it (#315) | `notification_router()` returns `None`, so `ErrorNotifier` keeps #17's single-target path — `notification_client()`, gated at `NOTIFICATION_SEVERITY_THRESHOLD`, cooldown keyed on the bare `error_code`. The two per-tier client providers are declared but never resolved, since the router is their only consumer |
...
**Severity routing changes the gate and the cooldown key, not the dispatch surface (#286).** With `NOTIFICATION_WARNING_THRESHOLD` set, `ErrorNotifier._effective_min_threshold()` lowers the alerting floor to `min(severity, warning)`, so a band that previously never alerted now does — and `NotificationRouter.resolve()` sends it to the warning client instead of the critical one. The cooldown key is tier-prefixed in that mode (`{tier}:{error_code}`), because a warning-tier 4xx sharing an `error_code` with a critical-tier 5xx would otherwise consume the incident alert's quiet window; `BaseCustomException` takes both `status_code` and `error_code` per instance, so that collision needs no subclass to reach. The two scopings compose, making the effective worker key `{tier}:{task_name}:{error_code}`. Unset, every one of these reduces to the #17 behaviour described above.
